...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Fill in user details wherever placeholders exist
  const nameEls = document.querySelectorAll(".user-name");
  const emailEls = document.querySelectorAll(".user-email");
  const balanceEls = document.querySelectorAll(".user-balance");

  nameEls.forEach(el => el.textContent = user.username || user.name || "User");
  emailEls.forEach(el => el.textContent = user.email || "");
  balanceEls.forEach(el => el.textContent = user.balance || "0.00");
});
